...
void MusicManager::save(string filename) {
    ...
    auto it_all = find_playlist(string("All"));
    auto it_tracks = (*it_all)->get_tracks();
    for (auto it = it_tracks.begin(); it < it_tracks.end(); it++) {
        string tmp = "";
        tmp += (*it)->get_title() + "," + (*it)->get_artist()
            + "," + (*it)->get_album() + "," + (*it)->get_mv_url();
        if ((*it)->get_playlists().size() > 0) {
            tmp += ",";
            for (auto k : (*it)->get_playlists()) {
                tmp += k->get_name() + ":";
            }
        }
        tmp += '\n';
